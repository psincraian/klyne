...
async def get_dashboard_overview(
    ...
    logger.info(
        f"Found {len(api_keys)} API keys for user {user_id} with package filter '{package_name}'"
    )
    if not api_keys:
        return []
    ...
    logger.info(
        f"Date range: {start_date} to {end_date} (converted to UTC: {start_datetime} to {end_datetime})"
    )
    ...
    sample_events_query = (
        ...
        .filter(AnalyticsEvent.api_key.in_(api_key_values))
    ...
    for event in sample_events:
        logger.info(
            f"Sample event: {event.package_name} - event_timestamp: {event.event_timestamp}, received_at: {event.received_at}"
        )
    ...
    if events_in_range == 0 and total_events > 0:
        ...
        logger.info(
            f"No events in 30-day range, extending to 90 days: {extended_start_date} to {end_date}"
        )
    ...
    for api_key in api_keys:
        ...
        stats_query = select(
            ...
            func.count(func.distinct(AnalyticsEvent.session_id)).label(
                "total_sessions"
            ),
            func.count(func.distinct(func.date(AnalyticsEvent.event_timestamp))).label(
                "active_days"
            ),
        ).filter(
            and_(
        ...
        logger.info(
            f"Running stats query for package {api_key.package_name} from {start_date} to {end_date}"
        )
        ...
        logger.info(
            f"Stats result for {api_key.package_name}: total_events={stats.total_events}, total_sessions={stats.total_sessions}, active_days={stats.active_days}"
        )
        ...
        # Get OS count
        os_query = select(func.count(func.distinct(AnalyticsEvent.os_type))).filter(
            and_(
                AnalyticsEvent.api_key == api_key.key,
                AnalyticsEvent.event_timestamp >= start_datetime,
            )
        ...
        avg_daily_events = float(stats.total_events or 0) / max(
            1, stats.active_days or 1
        )
...
async def get_timeseries_data(
    ...
    daily_stats_query = (
        ...
        .group_by(
            func.date(AnalyticsEvent.event_timestamp), AnalyticsEvent.package_name
        )
...
async def get_python_version_distribution(
    ...
    for stat in python_stats:
        event_percentage = (
            (stat.total_events / total_events * 100) if total_events > 0 else 0
        )
        session_percentage = (
            (stat.total_sessions / total_sessions * 100) if total_sessions > 0 else 0
        )
...
async def get_package_version_adoption(
    ...
    api_key_query = select(APIKey).filter(
        and_(APIKey.user_id == user_id, APIKey.package_name == package_name)
    )
    ...
    logger.info(
        f"Fetching package version adoption for {package_name} from {start_date} to {end_date}"
    )
